	...
	<h2 id="b2logos_page_title">Do you want to restore old data?</h2>
	...
	if($_POST["submitButton"])
	{
		if($_POST["restoreDataAcceptCheckbox"]=='accept') {
			...
			if($_POST["restoreDataRadio"]=='yes') {
				...
				$wpdb->query( "UPDATE $wpdb->term_taxonomy SET taxonomy = 'b2logocategory' WHERE taxonomy = 'clientscategory' ");
				...
				if ($clients_query->have_posts()) {
					...
					while ($i < $clients_query->post_count) {
						...
						$client = array('ID' => $post[$i]->ID, 'post_type' => 'b2logo');
				...
				update_option('b2logos_data_restored', stripslashes('yes'));
				
				echo '<div class="successMessage">Data was restored successfuly.</div>';
				
			}
			else if($_POST["restoreDataRadio"]=='no') {
				update_option('b2logos_data_restored', stripslashes('no'));
